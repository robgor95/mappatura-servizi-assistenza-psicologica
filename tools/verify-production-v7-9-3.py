...
O=Path(os.environ.get('QA_OUT','/tmp/v793-production'));O.mkdir(parents=True,exist_ok=True)
...
def fetch(path):
 req=Request(origin+'/'+path+'?verify_v793='+commit,headers={'User-Agent':'LazioReleaseVerification/7.9.3 (own site; no personal queries)','Cache-Control':'no-cache'})
 try:
...
for attempt in range(40):
 ...
 time.sleep(10)
checks.append({'name':'Production serves the exact committed V7.9.3 manifest','passed':deployed,'observed':last})
if deployed:
 pages=json.loads((R/'downloads/Audit_UX_Mappa_V7_8.json').read_text())['pages']
 paths=list(dict.fromkeys(pages+['index.html','version.json','robots.txt','sitemap.xml','data/audit_operativo_v7_9_2.json','data/audit_operativo_v7_9_3.json','data/presidi_geo_v7_9_2.json','data/portal_data_v7_3.json','data/privati_v7_5.json','data/multisede_v7_7_5.json','assets/audit-data-v7-9-2.js','assets/audit-data-v7-9-3.js','assets/servizi-v7-9-3.js','assets/map-data-v7-9-2.js','assets/map-v7-9-1.js','assets/directory-v7-9-2.js','downloads/Verifiche_V7_9_3.json','downloads/Audit_Operativo_V7_9_3.json','downloads/Release_Notes_V7_9_3.md']))
 for path in paths:
  try:
 ...
 try:
  status,headers,body,final=fetch('__audit-v793-not-found__');headers={k.lower():v for k,v in headers.items()}
...
report={'version':'7.9.3','checked_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'commit':commit,'production_origin':origin,'checks':checks,'passed':sum(c['passed'] for c in checks),'total':len(checks),'entrances_verified':False,'external_map_requests':0}
