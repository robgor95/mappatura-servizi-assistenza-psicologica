...
try{
  ...
  await test('Map starts with all 440 services, honest geographic coverage, no external requests until activation',async()=>{
    ...
    assert.match(await p.locator('#map-count').innerText(),/440 servizi/);assert.equal(req.filter(x=>!x.startsWith(BASE)).length,0);assert.equal(await p.evaluate(()=>window.qaGeoCalls),0);
  ...
  await test('Legacy service URL and empty reset remain functional',async()=>{
    ...
    await p.locator('#svc-q').fill('inesistente_qa');await p.waitForTimeout(500);assert.equal(await p.locator('#svc-empty').isVisible(),true);assert.equal(await p.locator('#svc-reset-empty').innerText(),'Mostra tutti i servizi');await p.locator('#svc-reset-empty').click();assert.match(await p.locator('#svc-count').innerText(),/440/);
  });
  ...
  await test('Missing geography shows an honest fallback, not fabricated map pins',async()=>{
    const {c:ct,p:pg}=await context();await ct.route('**/data/presidi_geo_v7_9.json',r=>r.fulfill({status:503,body:'unavailable'}));await pg.goto(BASE+'/mappa.html',{waitUntil:'networkidle'});await pg.locator('#map-controls:not([disabled])').waitFor();assert.match(await pg.locator('#map-count').innerText(),/440 senza posizione/);assert.equal(await pg.locator('#map-partial').isVisible(),true);await ct.close();
  });
...
const report={version:'7.9',tested_at:new Date().toISOString(),base:BASE,osm_tiles:'mocked; no external map-tile requests during automated checks',tests,passed:tests.filter(x=>x.passed).length,total:tests.length,screens};await fs.writeFile(OUT+'/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(report.passed!==report.total)process.exitCode=1;
