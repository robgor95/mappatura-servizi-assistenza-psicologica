...
const report={version:'7.10.2',tested_at:new Date().toISOString(),base:BASE,osm_tiles:'mocked; no external map-tile requests during automated checks',tests,passed:tests.filter(x=>x.passed).length,total:tests.length,screens};await fs.writeFile(OUT+'/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(report.passed!==report.total)process.exitCode=1;
