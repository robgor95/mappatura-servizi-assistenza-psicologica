...
O=Path(os.environ.get('QA_OUT','/tmp/v794-production'));O.mkdir(parents=True,exist_ok=True)
...
def fetch(path):
 req=Request(origin+'/'+path+'?verify_v794='+commit,headers={'User-Agent':'LazioReleaseVerification/7.9.4 (own site; no personal queries)','Cache-Control':'no-cache'})
 try:
...
for attempt in range(40):
 ...
 time.sleep(10)
checks.append({'name':'Production serves the exact committed V7.9.4 manifest','passed':deployed,'observed':last})
if deployed:
 pages=json.loads((R/'downloads/Audit_UX_Mappa_V7_8.json').read_text())['pages']
 paths=list(dict.fromkeys(pages+['index.html','version.json','robots.txt','sitemap.xml','data/audit_operativo_v7_9_4.json','data/presidi_geo_v7_9_4.json','data/portal_data_v7_3.json','data/privati_v7_5.json','data/multisede_v7_7_5.json','assets/audit-data-v7-9-4.js','assets/servizi-v7-9-4.js','assets/map-data-v7-9-4.js','assets/map-v7-9-1.js','assets/directory-v7-9-4.js','downloads/Verifiche_V7_9_4.json','downloads/Audit_Operativo_V7_9_4.json','downloads/Release_Notes_V7_9_4.md','downloads/Report_Geografia_V7_9_4.md','downloads/Audit_Multisede_V7_9_4.json','downloads/Audit_Multisede_V7_9_4.md','downloads/Coda_Geografia_V7_9_4.csv'])))
 for path in paths:
  try:
 ...
 try:
  status,headers,body,final=fetch('__audit-v794-not-found__');headers={k.lower():v for k,v in headers.items()}
 ...
 base='2ad2126b5c972ce49956607e368fc45b3fc4f7e6';allowed={'README.md','CHANGELOG.md','version.json','servizi.html','archivio.html','mappa.html','strutture-approfondite.html','documenti.html','.github/workflows/verify-v7-9-3.yml'}
...
report={'version':'7.9.4','checked_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'commit':commit,'production_origin':origin,'checks':checks,'passed':sum(c['passed'] for c in checks),'total':len(checks),'entrances_verified':False,'external_map_requests':0}
