...
check('version and clinical search count stay stable',()=>{assert.equal(v.web_version,'7.10.2');assert.equal(v.counts_current.search,443);assert.equal(v.map.services,443);assert.equal(v.map.localized,315);assert.equal(v.map.unlocated,128);assert.equal(v.indexing_enabled,false);});
...
const report={version:'7.10.2',tests,passed:tests.filter(x=>x.passed).length,total:tests.length};console.log(JSON.stringify(report,null,2));if(report.passed!==report.total)process.exitCode=1;
