...
for(const f of ['servizi-data-v7-5-1','audit-data-v7-9','audit-data-v7-9-1'])vm.runInContext(fs.readFileSync('assets/'+f+'.js','utf8'),sandbox);
...
sandbox.LazioAudit79.set(read('data/audit_operativo_v7_9.json'));sandbox.LazioAudit791.set(read('data/audit_operativo_v7_9_1.json'));
const rows=sandbox.LazioServices.build(data,read('data/privati_v7_5.json'));
fs.writeFileSync(process.argv[2]||'/tmp/current-rows-v791.json',JSON.stringify(rows,null,2));console.log(rows.length+' services');
