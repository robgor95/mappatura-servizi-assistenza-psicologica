...
check('Rieti accessibility is building-level and qualified',()=>{for(const k of ['rete:RI-03','rete:RI-06','rete:RI-12']){const r=by[k];assert.match(r.raw.accessibilita,/abbattimento barriere architettoniche/i);assert.match(r.raw.accessibilita,/non.*verifica/i);}});
