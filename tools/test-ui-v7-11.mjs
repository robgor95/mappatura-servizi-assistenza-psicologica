...
try{
 ...
 await test('Current map exposes the exact 443 / 330 / 113 service counts and uncertainty',async()=>{
  await p.goto(BASE+'/mappa.html',{waitUntil:'networkidle'});await p.locator('#map-controls:not([disabled])').waitFor();const text=await p.locator('#map-count').innerText();assert.match(text,/443 servizi/);assert.match(text,/330 localizzati/);assert.match(text,/113 senza posizione/);
  const d=await p.evaluate(async()=>{const x=await LazioMapData.load();return {count:x.rows.length,points:x.rows.filter(r=>LazioMapData.position(r,x.geo)).length};});assert.deepEqual(d,{count:443,points:330});
 });
 ...
 await test('Current structure directory loads the new layer and retains all source provenance',async()=>{
  ...
  await test('V7.9.3 exposes current SerD Nomentana access and transport without changing map counts',async()=>{
    ...
    await p.goto(BASE+'/mappa.html',{waitUntil:'networkidle'});await p.locator('#map-controls:not([disabled])').waitFor();const mapText=await p.locator('#map-count').innerText();assert.match(mapText,/443 servizi/);assert.match(mapText,/330 localizzati/);assert.match(mapText,/113 senza posizione/);
  });
...
const report={version:'7.11',tested_at:new Date().toISOString(),base:BASE,osm_tiles:'mocked; no external map-tile requests during automated checks',tests,passed:tests.filter(x=>x.passed).length,total:tests.length,screens};await fs.writeFile(OUT+'/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(report.passed!==report.total)process.exitCode=1;
