...
try{
  ...
  await test('Map starts with all 442 services, honest geographic coverage, no external requests until activation',async()=>{
    ...
    assert.match(await p.locator('#map-count').innerText(),/442 servizi/);assert.equal(req.filter(x=>!x.startsWith(BASE)).length,0);assert.equal(await p.evaluate(()=>window.qaGeoCalls),0);
  ...
  await test('Legacy service URL and empty reset remain functional',async()=>{
    ...
    await p.locator('#svc-q').fill('inesistente_qa');await p.waitForTimeout(500);assert.equal(await p.locator('#svc-empty').isVisible(),true);assert.equal(await p.locator('#svc-reset-empty').innerText(),'Mostra tutti i servizi');await p.locator('#svc-reset-empty').click();assert.match(await p.locator('#svc-count').innerText(),/442/);
  });
  ...
  await test('Missing geography shows an honest fallback, not fabricated map pins',async()=>{
    const {c:ct,p:pg}=await context();await ct.route('**/data/presidi_geo_v7_9_1.json',r=>r.fulfill({status:503,body:'unavailable'}));await pg.goto(BASE+'/mappa.html',{waitUntil:'networkidle'});await pg.locator('#map-controls:not([disabled])').waitFor();assert.match(await pg.locator('#map-count').innerText(),/442 senza posizione/);assert.equal(await pg.locator('#map-partial').isVisible(),true);await ct.close();
  });
 ...
 await p.goto(BASE+'/strutture-approfondite.html',{waitUntil:'networkidle'});assert.match(await p.locator('#result-count').innerText(),/183/);await p.locator('#directory-search').fill('FEBO');await p.waitForTimeout(500);assert.match(await p.locator('#directory-grid').innerText(),/FEBO/);
 });
 ...
 await test('All 176 V7.9 protected files still have exact baseline bytes',async()=>{
  const a=JSON.parse(await fs.readFile('downloads/Audit_Iniziale_V7_9_1.json','utf8'));assert.equal(a.protected_paths.length,176);for(const file of a.protected_paths)assert.equal(crypto.createHash('sha256').update(await fs.readFile(file)).digest('hex'),a.baseline_files[file].sha256,file);
 });
 await test('Current map exposes the exact 442 / 307 / 135 service counts and uncertainty',async()=>{
  await p.goto(BASE+'/mappa.html',{waitUntil:'networkidle'});await p.locator('#map-controls:not([disabled])').waitFor();const text=await p.locator('#map-count').innerText();assert.match(text,/442 servizi/);assert.match(text,/307 localizzati/);assert.match(text,/135 senza posizione/);
  const d=await p.evaluate(async()=>{const x=await LazioMapData.load();return {count:x.rows.length,points:x.rows.filter(r=>LazioMapData.position(r,x.geo)).length};});assert.deepEqual(d,{count:442,points:307});
 });
 await test('Gabbiano preserves twelve contracted legacy modules and two distinct Villa Pia services',async()=>{
  await p.goto(BASE+'/servizi.html?q=Gabbiano',{waitUntil:'networkidle'});await p.locator('#svc-controls:not([disabled])').waitFor();assert.match(await p.locator('#svc-count').innerText(),/^14 di 442/);
...
const report={version:'7.9.1',tested_at:new Date().toISOString(),base:BASE,osm_tiles:'mocked; no external map-tile requests during automated checks',tests,passed:tests.filter(x=>x.passed).length,total:tests.length,screens};await fs.writeFile(OUT+'/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(report.passed!==report.total)process.exitCode=1;
