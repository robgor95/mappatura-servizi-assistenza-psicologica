...
function detail(r){
  ...
  let body=section('Dove si trova',field('Tipo di servizio',r.subtype,true)+field('Comune',r.town)+field('ASL / territorio',r.asl)+field('Indirizzo',r.address,true)+field('Precisazioni sulla sede',v.nota_indirizzo||v.sede_dettaglio,true)+field('Gestione',managementLabel(v.gestore||v.gestione||v.natura),true)+field('Sede / dettaglio',v.sede_dettaglio||v.struttura_id,true)+field('Stato del servizio',v.stato_servizio,true)+field('Nota indirizzo',v.nota_indirizzo,true)+field('Audit multisede',v.nota_multisede_v794,true));
  body+=section('Come si accede',field('Accesso / prenotazione',v.accesso||v.ammissione||v.prenotazione,true)+field('Requisiti o limitazioni',v.criteri_ammissione||v.criteri_limitazioni,true)+field('Invio del servizio territoriale',v.modalita_invio,true)+field('Documenti richiesti',v.documenti_ingresso,true)+field('Durata indicata',v.durata_percorso,true)+field('Mobilità nel Lazio',v.mobilita_intraregionale||v.accesso_altre_asl,true)+field('Accesso da altre regioni',v.accesso_extraregionale||v.accesso_fuori_lazio_stato,true)+field('Precisazioni sull’accesso',v.nota_accesso_v793,true),'svc-accesso');
  ...
  body+=section('Contatti e orari',field('Telefono',v.telefono||v.contatti,true)+field('Email',v.email||(isPrivate?r.email:''),true)+field('Orari',v.orari,true)+field('Trasporto pubblico',v.trasporto_pubblico,true)+field('Recapito: tipo e limiti',v.note_contatti||v.nota_contatti,true)+field('Sito ufficiale',v.sito_ufficiale,true)+field('Nota sui recapiti',v.note_contatti||v.nota_contatti,true)+field('Recapito di sede in documento precedente',v.telefono_sede_documentato,true)+field('Email di sede documentata',v.email_sede_documentata,true));
  const ssnRaw=v.ssn_evidence_v79==='da-verificare'?(v.contratto_ssn_stato||'Da verificare per questa unità'):(v.rapporto_ssn||v.convenzione_ssn||v.contratto_ssn_stato);
  body+=section('Costi e rapporto con il SSN',field('Inquadramento',originLabel(r),true)+field('Rapporto con il SSN',ssnRaw||(r.origin==='rete'?'Servizio della rete pubblica; modalità e costi dipendono dal percorso di accesso.':'Da verificare con la struttura e nelle fonti.'),true)+field('Autorizzazione',v.autorizzazione_stato||v.autorizzazione_sanitaria,true)+field('Accreditamento',v.accreditamento_stato||v.accreditamento,true)+field('Contratto / convenzione',v.contratto_ssn_stato||v.convenzione_ssn,true)+field('Costi',v.costi,true)+field('Gratuità',v.gratuita,true)+field('Precisazione sul rapporto SSN',v.nota_rapporto_ssn,true)+field('Finanziamento / budget 2026',v.budget_2026_stato,true)+field('Nota budget 2026',v.nota_budget_2026,true)+field('Periodo / condizioni riportate',v.contratto_periodo||v.periodo_evidenza||v.condizioni_extraregionale,true)+field('Nota','Autorizzazione, accreditamento e convenzione non sono sinonimi. Se questa informazione è importante per il tuo accesso, chiedi conferma alla struttura.',true));
  body+=section('Capienza documentata',field('Posti autorizzati',v.posti_autorizzati)+field('Posti accreditati',v.posti_accreditati)+field('Posti contrattualizzati',v.posti_contrattualizzati)+field('Posti dichiarati',v.posti_dichiarati)+field('Precisazioni e discordanze',v.nota_posti,true)+field('Disponibilità attuale','Non rilevata: capienza e posti liberi non sono la stessa cosa.',true));
  ...
  body+=section('Aggiornamento e fonti',field('Data documentale originaria (non aggiorna i campi non riesaminati)',r.date||'Non documentato')+field('Note utili',[v.nota_temporale_v7,v.note,v.note_modulo,v.note_v7_2,v.note_v6].filter(Boolean).join('\n'),true));
  if(r.v791)body+=section('Riesame operativo V7.9.1',field('Campi riesaminati',r.v791.fields.join(', '),true)+field('Data del controllo documentale',r.v791.checked_at)+field('Ambito e limiti',r.v791.scope,true)+field('Note della verifica',r.v791.note,true));
  if(r.v792)body+=section('Riesame operativo V7.9.2',field('Campi riesaminati',r.v792.fields.join(', '),true)+field('Data del controllo documentale',r.v792.checked_at)+field('Ambito e limiti',r.v792.scope,true)+field('Note della verifica',r.v792.note,true));
  if(r.v793)body+=section('Riesame operativo V7.9.3',field('Campi riesaminati',r.v793.fields.join(', '),true)+field('Data del controllo documentale',r.v793.checked_at)+field('Ambito e limiti',r.v793.scope,true)+field('Note della verifica',r.v793.note,true));
...
function wire(){
  ...
  document.addEventListener('click',e=>{
    ...
    const target=e.target.closest('a[data-open]');if(target){e.preventDefault();returnKey=target.dataset.open;commit({...state,scheda:returnKey});if(target.dataset.section==='accesso'){$('svc-accesso').scrollIntoView({block:'start'});$('svc-accesso').focus({preventScroll:true});}}
...
async function start(){
  const [base,extra,multisite,audit,current,current2,current3,current4,current711]=await Promise.allSettled([
  ...
  if(extra.status==='fulfilled'&&multisite.status==='fulfilled'&&audit.status==='fulfilled'&&current.status==='fulfilled'&&current2.status==='fulfilled'&&current3.status==='fulfilled'&&current4.status==='fulfilled'&&current711.status==='fulfilled')$('svc-load-status').hidden=true;
  else{
    const missing=[];
