/* V7.9.2 additive field overlay; all preceding snapshots remain immutable. */
(function(root){
'use strict';
const A=root.LazioServices,copy=x=>JSON.parse(JSON.stringify(x));let overlay=null;
function validate(d){
 if(!d||d.version!=='7.9.2'||!Array.isArray(d.revisions)||!Array.isArray(d.additions))throw Error('Overlay corrente non valido');
...
function raw(r,e){const v=copy(r);Object.assign(v,copy(e.fields||{}));v._riesame_v792={checked_at:e.checked_at,fields:Object.keys(e.fields||e.record||{}),sources:e.sources,note:e.note,scope:e.scope};return v;}
function annotate(r,e){
 const v=r.raw;r.v792={checked_at:e.checked_at,fields:Object.keys(e.fields||e.record||{}),sources:e.sources,note:e.note||'',scope:e.scope};r.auditDate=e.checked_at;r.sources=[...new Set(r.sources.concat(e.sources))];
 ...
 if(r.origin!=='rete'){r.auth=A.evidence(r.admin.autorizzazione);r.accreditation=A.evidence(r.admin.accreditamento);r.ssn=['indicata','dichiarazione','da-verificare'].includes(v.ssn_evidence_v792)?v.ssn_evidence_v792:r.ssn;}
...
function directory(category,rows,d){
 ...
 const rev=new Map(d.revisions.map(e=>[e.service_key,e]));const out=rows.map(r=>{
  ...
  v.riesame_v792='Parziale: '+e.checked_at;v.fonti_v792=e.sources.join(' | ');v.note_v792=e.note;
  return v;
 });const known=new Set(out.map(r=>String(r.id_modulo||r.id)));
 for(const e of d.additions){if(e.origin==='privati')continue;const id=e.service_key.split(':')[1];if(known.has(id))throw Error('Duplicazione nella directory');const v=copy(e.record);v.struttura=v.denominazione||v.nome;v.tipologia_modulo=v.tipo||v.modulo;v.sede=v.indirizzo;v.riesame_v792='Nuovo servizio: '+e.checked_at;v.fonti_v792=e.sources.join(' | ');v.note_v792=e.note;out.push(v);known.add(id);}return out;
}
root.LazioAudit792={set,validate,directory};
