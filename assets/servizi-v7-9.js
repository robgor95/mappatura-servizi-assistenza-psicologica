...
function detail(r){
  ...
  let body=section('Dove si trova',field('Tipo di servizio',r.subtype,true)+field('Comune',r.town)+field('ASL / territorio',r.asl)+field('Indirizzo',r.address,true)+field('Gestione',managementLabel(v.gestore||v.gestione||v.natura),true)+field('Sede / dettaglio',v.sede_dettaglio||v.struttura_id,true)+field('Stato del servizio',v.stato_servizio,true)+field('Nota indirizzo',v.nota_indirizzo,true));
...
async function start(){
  const [base,extra,multisite,audit]=await Promise.allSettled([
  ...
  if(extra.status==='fulfilled'&&multisite.status==='fulfilled'&&audit.status==='fulfilled')$('svc-load-status').hidden=true;
  else{
    const missing=[];
