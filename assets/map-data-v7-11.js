...
async function load(){
  const result=await Promise.allSettled(['/data/portal_data_v7_3.json','/data/privati_v7_5.json','/data/multisede_v7_7_5.json','/data/presidi_geo_v7_11.json','/data/audit_operativo_v7_9.json','/data/audit_operativo_v7_9_1.json','/data/audit_operativo_v7_9_2.json','/data/audit_operativo_v7_9_3.json','/data/audit_operativo_v7_9_4.json','/data/audit_operativo_v7_11.json'].map(json));
  ...
  if(result[9].status==='fulfilled')root.LazioAudit711.set(result[9].value);else missing.push('correzioni operative V7.11: dati precedenti da riconfermare');
...
function quality(p){
 ...
 if(p.quality==='C'&&p.precision==='building')return 'C · Edificio o presidio indicativo; reparto e ingresso non verificati';
