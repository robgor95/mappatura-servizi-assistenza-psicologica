...
function update(){
  ...
  const points=filtered.filter(r=>G.position(r,geo)),street=points.filter(r=>G.position(r,geo).quality==='C');
  $('map-count').textContent=filtered.length+' servizi: '+points.length+' localizzati ('+street.length+' con posizione indicativa), '+(filtered.length-points.length)+' senza posizione univoca.';
...
function drawList(){
  const fragment=document.createDocumentFragment();filtered.slice(0,shown).forEach(row=>{
    ...
    li.append(heading,element('p',row.subtype+' · '+row.town),element('p',row.address),element('p',G.quality(p||geo.records[row.key]),{class:'map-quality '+(p&&p.quality==='C'?'approx':'')}));
    if(row.serviceState)li.append(element('p','Stato: '+row.serviceState,{class:'micro','data-service-state':''}));
...
function popup(group){
  ...
  group.forEach(({row,p})=>{const section=element('div',null,{class:'map-popup-item'});
    section.append(element('h3',row.name),element('p',row.subtype),element('p',row.address+' · '+row.town),element('p',G.quality(p||geo.records[row.key])));
    section.append(link('Dettagli e contatti',G.serviceLink(row,state)));
    if(p.note)section.append(element('p',p.note,{class:'micro'}));
...
function drawMarkers(){
  ...
  groups.forEach(group=>{
    const count=group.length,p=group[0].p,approx=group.every(x=>x.p.quality==='C');
    const center=[group.reduce((n,x)=>n+x.p.lat,0)/count,group.reduce((n,x)=>n+x.p.lng,0)/count];
    const text=count>1?String(count):(p.quality==='C'?'~':p.quality),icon=L.divIcon({className:'presidio-marker'+(approx?' approximate':''),html:'<span>'+text+'</span>',iconSize:[38,38],iconAnchor:[19,19]});
...
function select(row){
  selected=row.key;const p=G.position(row,geo);$('map-selected').replaceChildren(element('strong',row.name),element('p',row.address+' · '+row.town),element('p',G.quality(p||geo.records[row.key])));
  if(!p){tell('Questo servizio non ha ancora una posizione univoca. Consulta indirizzo, fonti e recapiti nella scheda; nessun pin viene inventato.');}
  $('map-selected').append(link('Apri scheda e contatti',G.serviceLink(row,state)));
  $('map-selected').hidden=false;
  if(map&&p){map.setView([p.lat,p.lng],p.quality==='C'?15:17,{animate:false});drawMarkers();const same=rows.filter(r=>{const q=G.position(r,geo);return q&&q.lat.toFixed(6)===p.lat.toFixed(6)&&q.lng.toFixed(6)===p.lng.toFixed(6);}).map(r=>({row:r,p:G.position(r,geo)}));
